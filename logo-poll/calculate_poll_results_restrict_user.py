...
RESTRICT_VOTING = True  # Set to True to restrict voting to users in user_poll_allowed.txt
ALLOWED_USERS_FILE = "/home/fred/xlibre_logo/user_poll_allowed.txt"  # File with allowed voters
...
def read_allowed_users():
    """Read allowed GitHub usernames from ALLOWED_USERS_FILE."""
    if not RESTRICT_VOTING:
        ...
        with open(ALLOWED_USERS_FILE, 'r') as f:
            for line in f:
                ...
                if line.startswith('@'):
                    ...
                    logger.warning(f"Invalid username format in {ALLOWED_USERS_FILE}: {line}")
        logger.info(f"Loaded {len(allowed_users)} allowed users from {ALLOWED_USERS_FILE}")
        return allowed_users
    except FileNotFoundError:
        logger.error(f"Allowed users file {ALLOWED_USERS_FILE} not found")
        return set()
    except Exception as e:
        logger.error(f"Error reading {ALLOWED_USERS_FILE}: {e}")
        return set()
...
def count_votes(comments, valid_logo_numbers):
    ...
    allowed_users = read_allowed_users() if RESTRICT_VOTING else None
    ...
    for comment in comments:
        ...
        try:
            ...
            if RESTRICT_VOTING and allowed_users is not None and user not in allowed_users:
                logger.warning(f"User {user} not in allowed list, vote for Logo {logo_num} ignored")
                warnings.append(f"User {user} not in allowed list, vote for Logo {logo_num} ignored")
                continue
...
def update_results_issue(vote_counts, valid_logo_numbers, warnings):
    ...
    if not vote_counts or not valid_logo_numbers:
        logger.error("No valid vote counts or logos to update results issue")
        return
...
def main():
    ...
    if vote_counts and valid_logo_numbers:
